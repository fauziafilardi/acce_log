          ...
          <div class="card">
            ...
            <div class="card__body">
              <b-row>
                <b-col sm="2">
                  <b-row>
                    <b-col sm="2" style="max-width:fit-content !important;">Year &nbsp;&nbsp;</b-col>
                    <b-col sm="10">
                ...
                <b-col sm="6">
                  <b-row>
                    <!-- <b-col style="max-width:fit-content !important;">Marketing &nbsp;&nbsp;</b-col> -->
                    <b-col sm="2">
                    ...
                    <b-col sm="5">
                      <div v-if="inputStatus=='new'">
                        <ACCDropDown
                          @change="Onco_marketingChange"
                          :prop="PI_co_marketing"
                          v-model="M_Target.marketing_name"
                          :label="M_Target.marketing_nameLabel"
                          :disabled="isDisableTable"
                          ref="ref_co_marketing1"
                        />
                      </div>
                      <div v-else>
                        <span>{{ M_Target.marketing_id && M_Target.marketing_id !== '' ? (M_Target.marketing_id + ' - ' + M_Target.marketing_name) : '' }}</span>
                      </div>
                    </b-col>
                  </b-row>
                </b-col>
...
export default {
  ...
  methods: {
    OnyearsChange(data) {
      this.$nextTick(() => {
        this.filterAction = data.id;
        this.filterAction = data.label;
      });
    },
    Onco_marketingChange(data) {
      this.$nextTick(() => {
        this.M_Target.co_marketing = data.id;
        this.M_Target.co_marketingLabel = data.label;
      });
    },
    doBack() {
      this.$router.go(-1);
    ...
    OnAccumulate(data, index) {
      ...
      var ftl = parseFloat(
        this.replaceAllString(item.ftl, ",", "", "number") !== ""
          ? this.replaceAllString(item.ftl, ",", "", "number")
          : 0
      );
      var ltl = parseFloat(
        this.replaceAllString(item.ltl, ",", "", "number") !== ""
          ? this.replaceAllString(item.ltl, ",", "", "number")
          : 0
      );
      var project = parseFloat(
        this.replaceAllString(item.project, ",", "", "number") !== ""
          ? this.replaceAllString(item.project, ",", "", "number")
          : 0
      );
      var rental = parseFloat(
        this.replaceAllString(item.rental, ",", "", "number") !== ""
          ? this.replaceAllString(item.rental, ",", "", "number")
          : 0
      );
    ...
    doGetList2() {
      ...
      this.CallFunction(param).then((response) => {
        ...
        for (let i = 0; i < this.items.length; i++) {
          var ftl =
            data[i] !== undefined
              ? data[i].ftl_amt && data[i].ftl_amt !== ""
                ? data[i].ftl_amt
                : 0
              : 0;
          var ltl =
            data[i] !== undefined
              ? data[i].ltl_amt && data[i].ltl_amt !== ""
                ? data[i].ltl_amt
                : 0
              : 0;
          var project =
            data[i] !== undefined
              ? data[i].project_amt && data[i].project_amt !== ""
                ? data[i].project_amt
                : 0
              : 0;
          var rental =
            data[i] !== undefined
              ? data[i].rental_amt && data[i].rental_amt !== ""
                ? data[i].rental_amt
                : 0
              : 0;
          ...
          this.items[i].total = this.isCurrency(
            ftl + ltl + project + rental,
            this.decimal
          );
        }
      });
    ...
    getYear() {
      ...
      this.CallFunction(param).then((response) => {
        ...
        this.filterAction =
          Object.keys(this.paramFromList).length > 0
            ? this.paramFromList.years
            : data[0].years;
        ...
        this.filterAction = new Date().getFullYear();
      });
    },
    getMarketing() {
      var param = {
      ...
      this.CallFunction(param).then((response) => {
        ...
        this.filterActionM =
          Object.keys(this.paramFromList).length > 0
            ...
            : data[0].marketing_id;
        this.M_Target.marketing_id =
          Object.keys(this.paramFromList).length > 0
            ? this.paramFromList.marketing_id
            : data[0].marketing_id;
        this.M_Target.marketing_name =
          Object.keys(this.paramFromList).length > 0
            ? this.paramFromList.marketing_name
            : data[0].marketing_name;
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          this.cmbMarketing.push({
            ...
            text: data[i].marketing_id + " - " + data[i].marketing_name,
          });
          this.PI_co_marketing.cOption.push({
            id: data[i].marketing_id,
            label: data[i].marketing_id + " - " + data[i].marketing_name,
          });
        }
      });
    },
