                    ...
                    <div class="card">
                        <div class="card__title" style="padding-bottom: 5px !important;">
                            <b-row>
                                ...
                                <b-col>
                                    <b-badge variant="primary">&nbsp;</b-badge> <span style="color: #7f8084;"> Prospect</span> &nbsp;
                                    <b-badge variant="success">&nbsp;</b-badge> <span style="color: #7f8084;"> Customer</span> &nbsp;
                                </b-col>
                                <b-col>
                                    <b-form-select
                                        id="cmbFilter"
                                ...
                                >
                                    ...
                                    <template v-slot:cell(customer_name)="data">
                                        <div v-if="data.item.customer_status == 'N'">
                                            <span class="badge-success badgeStatus">New</span> {{data.item.customer_name}}
                                        </div>
                                        <div v-if="data.item.customer_status == 'G'">
                                            <span class="badge-primary badgeStatus">Negotiation</span> {{data.item.customer_name}}
                                        </div>
                                        <div v-if="data.item.customer_status == 'M'">
                                            <span class="badge-danger badgeStatus">Maintain</span> {{data.item.customer_name}}
                                        </div>
                                    </template>

                                    <template v-slot:cell(last_action)="data">
                                        {{data.item.last_action && data.item.last_action !== '' ? data.item.last_action : '-'}}
                                    </template>

                                    <template v-slot:cell(action)="data">
                                        <div class="ChartLegend__Content" :style="'background-color: '+(data.item.action == '1' ? 'blue' : 'green')+'; width: 10px; height: 10px; padding: 10px; border-radius: 10px;'"></div>
                                        <div>
                                            <font-awesome-icon style="color: blue;" :icon="data.item.action=='C' ? 'phone-square-alt' : (data.item.action=='V' ? 'hand-paper' : 'utensils')" size="lg" />
