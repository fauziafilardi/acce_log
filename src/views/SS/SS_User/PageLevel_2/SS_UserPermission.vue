                ...
                <b-row v-show="isShow" style="padding-left: 10px; padding-bottom: 10px; !important;">
                    <b-col md="12" id="col-mid" class="bColMasterForm">
                        <b-container class="bv-example-row">
                            <div v-for="(module) in allModules" v-bind:key="module.module_seq">
                                <!-- Module -->
                                <b-row :no-gutters="true">
                                    <b-col cols="auto"> <span @click="expand('M', module.module_seq)" class="module" :id="'module_'+module.module_seq"><font-awesome-icon :icon="module.icon?module.icon:'plus-circle'" class="icon-expand" /></span> </b-col>
                                ...
                                <b-row :no-gutters="true" v-show="showEvent == module.module_seq">
                                    <b-container class="bv-example-row" v-show="onFilteredEvents.length > 0">
                                        <div v-for="(event) in onFilteredEvents" v-bind:key="event.event_seq">
                                            <!-- Event -->
                                            <b-row style="margin-left: 26px" :no-gutters="true">
                                                <b-col cols="auto"> <span @click="expand('E', event.event_seq)" class="event" :id="'event_'+event.event_seq"><font-awesome-icon :icon="event.icon?event.icon:'plus-circle'" class="icon-expand" /></span> </b-col>
                                            ...
                                            <b-row :no-gutters="true" v-show="showOption == event.event_seq">
                                                <b-container class="bv-example-row" v-show="onFilteredOptions.length > 0">
                                                    ...
                                                    <tr v-for="(option) in onFilteredOptions" v-bind:key="option.option_seq">
                                                        <!-- Option -->
...
export default {
    ...
    data() {
        return {
            ...
            showEvent: null,        // Isinya module_seq
            showOption: null,       // Isinya event_seq
            ...
            this.allOptions.forEach(el => {
                if (
                    ...
                    dataPost.push({
                        ...
                        print_status: el.print==true?1:0,
                        view_status: el.view==true?1:0
                    })
                }
            })
                ...
                if (data.length < 1) {
                    ...
                    this.$nextTick(() => {
                        data.forEach(element => {
                            var idx = allO.map(el => { return el.option_seq }).indexOf(element.option_seq)
                            if (idx > -1) {
                                ...
                                var idxEvent = allE.map(el => { return el.event_seq }).indexOf(this.allOptions[idx].event_seq)
                                this.allEvents[idxEvent].set = true

                                var idxModule = allM.map(el => { return el.module_seq }).indexOf(this.allOptions[idx].module_seq)
