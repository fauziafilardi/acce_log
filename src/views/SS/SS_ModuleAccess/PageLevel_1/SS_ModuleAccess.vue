                                ...
                                <b-container class="bv-example-row">
                                    <div v-for="(menu) in onFilteredMenus" v-bind:key="menu.page_master_seq">
                                        ...
                                        <b-row style="margin-left: 20px" :no-gutters="true">
                                            ...
                                            <b-col cols="auto"> <input style="vertical-align:middle !important" type="checkbox" name="cb_menu" :id="'P_'+menu.page_master_seq" :checked="menu.set" v-model="menu.set" :disabled="menu.disabled" > </b-col>
                                            <b-col cols="auto"> <span>{{ menu.text_menu }}</span> </b-col>
                                        </b-row>
                                        <!-- End Menu -->
                                    </div>
                                </b-container>
    ...
    methods: {
        ...
        M_Delete(from){
            if (from == 'M') {
                ...
                if (this.eventAccessSelected == null) {
                    ...
                    .then((ress) => {
                        ...
                        if (ress.value) {
                            var param = {
                                OptionSeq: this.getOptionSeq().OptionSeq,
                                LineNo: 1,
                                module_seq: this.eventAccessSelected.module_seq,
                                event_seq: this.eventAccessSelected.value,
            ...
            if(this.$store.getters.getStatus.toUpperCase() == 'MODULEENTRY'){
                ...
                if(this.$store.getters.getStatus.toUpperCase() !== 'NEW'){
                    ...
                    var param = {
                        ...
                        menu_name: this.M_SS_ModuleAccess.MenuName,
                        short_name: this.M_SS_ModuleAccess.ShortName,
                    }
                ...
                if(this.$store.getters.getStatus.toUpperCase() !== 'NEW'){
                    ...
                    var param = {
                        ...
                        LineNo: 1,
                        user_input: this.getDataUser().user_id,
                        text_menu: this.M_SS_EventAccess.TextMenu,
                        module_seq: this.M_SS_EventAccess.ModuleSeq,
                        // event_seq: 0
                    }
                    this.M_SS_EventAccess.UserInput = this.getDataUser().user_id
                    ...
                    this.$refs.modalEventAccess.hide()
                }
            }
                ...
                .then((ress) => {
                    ...
                    if (ress.value) {
                        ...
                        this.AllPage.forEach(element => {
                            if (element.set == true) {
                                if (! this.OptionInUse.map(e => e.value).includes(element.page_master_seq)) {
                                    dataOption.push({
                                        page_master_seq: element.page_master_seq
                                    })
                                }
                            }
                        })
            ...
            this.postJSON( this.getUrlById(), param )
                ...
                if(data.length > 0){
                    this.$nextTick(() => {
                        ...
                        this.AllModules = [...new Set(data.map(x => x.module_cd.toUpperCase()))]
                    })
            ...
            if(this.eventOption.length > 0){
                this.$nextTick(() => {
                    ...
                    this.eventOption.forEach(element => {
                        var idx = allDatas.map(el => el.page_master_seq).indexOf(element.value)
        ...
        expand (param) {
            if(param == this.selectedModuleForMenu){
                ...
                this.onFilteredMenus = this.AllPage.filter(val => { return val.module_cd.toUpperCase() == param })
        ...
        getEventAccess (data) {
             var param = {
                OptionSeq: this.getOptionSeq().OptionSeq,
                LineNo: 1,
                module_seq: data.value
            }

            this.M_SS_EventAccess.ModuleSeq = data.value
