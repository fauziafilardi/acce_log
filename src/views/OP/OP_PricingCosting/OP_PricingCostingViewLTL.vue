            ...
            <div class="card__title">
              <b-row>
                <b-col style="max-width:fit-content !important;">
                  <span>View Standard Pricing & Costing LTL</span>
                </b-col>
                    ...
                    <b-row>
                      ...
                      <div class="card__body">
                        <div class="table--list" :id="'marketingmaster'">
                          ...
                          >
                            <template v-slot:cell(margin_kgs)="data">
                              <span style="text-align:center;">
                                {{isCurrency(data.item.margin_kgs,decimal)}}
                                <b>({{data.item.margin_percent_kgs}}%)</b>
                              </span>
                            </template>
                            <template v-slot:cell(margin_cbm)="data">
                              <span style="text-align:center;">
                                {{isCurrency(data.item.margin_cbm,decimal)}}
                                <b>({{data.item.margin_percent_cbm}}%)</b>
                              </span>
                            </template>

                            <template v-slot:cell(selling_price_kgs)="data">
                              <span
                                style="text-align:rigth;"
                              >{{isCurrency(data.item.selling_price_kgs,decimal)}}</span>
                            </template>
                            <template v-slot:cell(selling_price_cbm)="data">
                              <span
                                style="text-align:rigth;"
                              >{{isCurrency(data.item.selling_price_cbm,decimal)}}</span>
                            </template>
                            <template v-slot:cell(cost_price_kgs)="data">
                              <span
                                style="text-align:rigth;"
                              >{{isCurrency(data.item.cost_price_kgs,decimal)}}</span>
                            </template>
                            <template v-slot:cell(cost_price_cbm)="data">
                              <span
                                style="text-align:rigth;"
                              >{{isCurrency(data.item.cost_price_cbm,decimal)}}</span>
                            </template>
                          </b-table>
                        </div>
                      </div>
  ...
  data() {
    return {
      ...
      fieldHeader: [
        ...
        {
          key: "selling_price_kgs",
          label: "Selling Price KGS",
          tdClass: "ContentACCList2 notranslate th-cus-center",
          thClass: "HeaderACCList2 S th-cus-center",
        },
        ...
        {
          key: "selling_price_cbm",
          label: "Selling Price CBM",
          tdClass: "ContentACCList2 notranslate th-cus-center",
          thClass: "HeaderACCList2 S th-cus-center",
        },
          ...
          for (let i = 0; i < response.Data.length; i++) {
            ...
            this.items2.push({
              ...
              value_cbm: this.isCurrency(
                response.Data[i].cost_value_cbm,
                this.decimal
              ),
              value_kgs: this.isCurrency(
                response.Data[i].cost_value_kgs,
                this.decimal
              ),
    ...
    GetDataBy() {
      ...
      this.getJSON(this.getUrlCRUD(), param).then((response) => {
        ...
        this.items.push({
          ...
          selling_price_kgs: data.kgs_selling_price__tb_7,
          period: data.period,

          cost_price_kgs: data.total_cost_kgs,
          margin_kgs: data.margin_ltl_kgs,
          selling_price_cbm: data.cbm_selling_price__tb_6,
          cost_price_cbm: data.total_cost_cbm,
          margin_cbm: data.margin_ltl_cbm,
          margin_percent_cbm: data.margin_percent_ltl_cbm,
          margin_percent_kgs: data.margin_percent_ltl_kgs,
        });
