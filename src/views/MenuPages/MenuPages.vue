    ...
    <div class="dashboard-page__body">
        <div class="yah">
            <b-col md="6">
                <div class="pull--left" style="width:100%;">
                    <b-form-input @keyup.enter.native="onFilterKeyUp" size="sm" placeholder="Filter" style="font-size: 12px" v-model="filter"></b-form-input>
                </div>
            </b-col>
            <b-col md="6">
                <div class="pull--right">
                    <span class="filtering__ex-col" style="font-size: 12px" @click="collapseAll">Collapse All</span> |
        ...
        <div class="yah" v-for="(event, indexEvent) in ev" :key="indexEvent" v-show="!onFiltered">
            <b-col>
                ...
                <div class="yah">
                    <b-collapse :visible="collapsed[indexEvent]" :id="'menuCollapseAll'+indexEvent" style="width: 100%">
                        <div class="yah">
                            <b-col md="6" v-for="(option, indexChild) in event.child" :key="indexChild">
                                <!-- <div @click.self="doCallMenu(option)"> -->
                                    <div class="name-yah" @click.self="doCallMenu(option)">
                                        ...
                                        <div v-if="option.fav === 0" class="pull-right list__icon-right" style="z-index: -1;">
                                            <i class="icon-star" @click="doInsert(event.menu_seq_no, option.option_seq, option.ps, indexChild)"></i>
                                        </div>
                                        <!-- selain 0 dia muncul bintang -->
                                        <div v-else class="pull-right list__icon-right is-favorite" style="z-index: -1;">
                                            <i class="icon-star" @click="doDelete(event.menu_seq_no, option.option_seq, indexChild)"></i>
                                        </div>
                                    </div>
                                <!-- </div> -->
                            </b-col>
        ...
        <!-- <div class="yah" v-show="onFiltered"> -->
          <div class="yah" v-show="onFiltered">
            <b-col md="6" v-for="(option, index) in menuFilter" :key="index" @click.self="doCallMenu(option)">
              <div class="name-yah" @click.self="doCallMenu(option)">
                {{option.menu_name}}
                <div v-if="option.fav === '0'" class="pull-right list__icon-right">
                  <i class="icon-star" @click="doInsert(option.menu_seq_no, option.option_seq, option.ps, index)"></i>
                </div>
...
export default {
  ...
  created() {
      ...
      this.modules = this.menus === undefined ? undefined : this.menus.module_list
  ...
  methods: {
    ...
    doInsert: function(menuSeqNo, optionSeq, ps, indexChild) {
      ...
      .then((response) => {
        ...
        this.$parent.$children[1].$children[3].changeFavMenu(localStorage.lsFavoriteMenu)
      })
    },
